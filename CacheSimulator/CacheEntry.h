...
public:
    ...
    {
        m_LineAddress = (UINT_PTR)lineAddress & ~(UINT_PTR)0x3f;
    }

    bool IsIncluding(void* addr)
    {
        assert(GetIndex(addr) == GetIndex((void*)m_LineAddress));

        UINT_PTR tag1 = GetTag((void*)m_LineAddress);
